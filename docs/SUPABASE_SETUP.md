...
`db/` 폴더에 스키마와 레벨별 시드 SQL이 분리되어 있습니다.
모두 **멱등(여러 번 실행 가능)** 하도록 작성되어 있습니다.

| 순서 | 파일 | 역할 |
|---|---|---|
| 1 | `db/schema.sql`  | 테이블 · RLS · 트리거 · 공식 덱 5개 |
| 2 | `db/n5_seed.sql` | N5 단어/예문 |
| 3 | `db/n4_seed.sql` | N4 단어/예문 |
| 4 | `db/n3_seed.sql` | N3 단어/예문 |
| 5 | `db/n2_seed.sql` | N2 단어/예문 |
| 6 | `db/n1_seed.sql` | N1 단어/예문 |

1. 좌측 메뉴 **SQL Editor** 진입
2. 위 순서대로 각 파일을 통째로 복사 → 새 탭에서 **Run**
3. 처음 한 번 실행하면 다음이 모두 생성됩니다:
...
⚠️ 시드 SQL은 해당 레벨 공식 덱의 기존 단어를 **DELETE 후 재삽입** 합니다.
연결된 `word_progress` 가 CASCADE 로 함께 삭제되므로, 처치 카운트를 유지하려면
해당 시드 파일을 다시 실행하지 말고 `db/schema.sql` 만 갱신하세요.
