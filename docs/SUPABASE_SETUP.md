...
`supabase_schema.sql` 한 파일에 스키마 · RLS · 트리거 · 공식 덱 · N5~N1 단어 +
예문 시드까지 전부 들어 있습니다. **재실행 가능**하도록 작성되어 있습니다.

1. 좌측 메뉴 **SQL Editor** 진입
2. `supabase_schema.sql` 파일을 통째로 복사
...
4. 다음이 모두 생성됩니다:
   ...
   - RLS 정책 활성화
   - 공식 덱 5개 (N5~N1) — 이미 있으면 스킵
   - 공식 단어 27개 + 예문 6개 — 매 실행마다 깨끗이 재시드

⚠️ 단어 시드 부분(파일 하단 `delete from public.words where deck_id in (...)`)은
공식 덱의 기존 단어를 **모두 삭제**한 뒤 재삽입합니다. 연결된 `word_progress` 가
CASCADE 로 같이 삭제되므로, 처치 카운트를 유지하려면 해당 블록만 주석 처리하세요.
